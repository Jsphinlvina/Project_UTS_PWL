...
@section('content')
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        ...
        <form method="post" action="/dashboard/polling-matakuliah/{{$datas->id_polling}}">
            @method('put')
            @csrf
            <div class="col-lg-5">
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control" id="start_at" name="start_at"
                           value="{{old('start_at', $datas->start_at)}}>
                </div>
                <div class=" mb-3">
                    <label for="end_at" class="form-label">Tanggal Selesai:</label>
                    <input type="date" class="form-control" id="end_at" name="end_at"
                           value="{{old('end_at', $datas->end_at)}}>
                </div>
                 <div class=" mb-3">
                    <label for="is_active" class="form-label">Status Aktif:</label>
                    <select class="form-control" id="is_active" name="is_active">
                        <option value="{{ $datas->is_active == 1 ? 'selected' : '' }}">Aktif</option>
                        <option value="{{ $datas->is_active == 0 ? 'selected' : '' }}">Tidak Aktif</option>
                    </select>
                </div>
            </div>
